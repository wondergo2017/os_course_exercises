...
2. **不同特权级有什么区别？**
   ...
   0为OS内核态，3为用户态执行应用程序，中间很少实现，一般用于驱动以及其他自定义程序
...
3. **请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。**
   ...
   系统调用的时候，比较CPL<=DPL（门）同时CPL>=DPL（段），才能执行

   中断响应的时候，比较Max(CPL,RPL)<=DPL（段）的时候才能执行
