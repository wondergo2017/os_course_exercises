...
1. **什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？**
   ...
   划分程序成为多个模块，使得同一时刻只有互斥模块的一部分在内存中，使得在较小的内存中可以跑更大的程序。

   程序开发者需要划分程序，给出程序模块结构。

2. **什么是交换技术？覆盖与交换有什么不同？**

   在程序不用的时候，将其换出，要用的时候再放回内存。
...
1. **什么是时间局部性、空间局部性和分支局部性？**
   ...
   时间局部性：在访问一个地址之后，在最近的时间内，很可能再次访问

   空间局部性：在访问一个地址之后，最近可能再次访问其附近的地址

   分支局部性：跳转后到达一个地址，很可能下一次跳转回到同样的地址
